 ...
 *
 * This class is the ModifyCustomerController class which implements the Initializable interface.
 * This class takes customer data from a selected customer and allows the user to edit and save it to the database
 *
 */
public class ModifyCustomerController implements Initializable {
    @FXML
    public TextField customerID_txt_ModifyCustomer;
    @FXML
    public TextField customerName_txt_ModifyCustomer;
    @FXML
    public TextField customerAddress_txt_ModifyCustomer;
    @FXML
    public TextField customerPhoneNumber_txt_ModifyCustomer;
    @FXML
    public TextField customerPostalCode_txt_ModifyCustomer;
    @FXML
    public ComboBox customerCountry_cb_ModifyCustomer;
    @FXML
    public ComboBox customerProvince_cb_ModifyCustomer;
    @FXML
    private Button save_modifyCustomer;
    @FXML
    private Button cancel_modifyCustomer;
    ...
    /**
     * the cancel_modifyCustomer method confirms that the user wants to cancel adding the data
     * then the method opens the AppointmentsMain page
     * @param event - cancel_modifyCustomer takes the event ActionEvent
     * @throws IOException - cancel_modifyCustomer throws IOException
     */
    @FXML
    public void cancel_modifyCustomer(ActionEvent event) throws IOException {
    ...
    public void selectionChange(ActionEvent event) throws IOException {
        try {
            customerProvince_cb_ModifyCustomer.setValue(null);
            if (customerCountry_cb_ModifyCustomer.getSelectionModel().getSelectedItem().toString() == "Canada"){
                ...
                customerProvince_cb_ModifyCustomer.setItems(statesObsList);
            }
            if (customerCountry_cb_ModifyCustomer.getSelectionModel().getSelectedItem().toString() == "UK"){
                ObservableList<String> statesObsList = FXCollections.observableArrayList(england, wales, scotland, northernireland);
                customerProvince_cb_ModifyCustomer.setItems(statesObsList);
            }
            if (customerCountry_cb_ModifyCustomer.getSelectionModel().getSelectedItem().toString() == "USA"){
    ...
    /**
     * the save_modifyCustomer method saves the user input to the database if all fields contain data
     * lists pertaining to customer data are edited as needed
     ...
     */
    @FXML
    public void save_modifyCustomer(ActionEvent event) throws IOException {
        try {
            String customerIDAsString = customerID_txt_ModifyCustomer.getText();
            int customerID = Integer.parseInt(customerIDAsString);
            String customerName = customerName_txt_ModifyCustomer.getText();
            String customerAddress = customerAddress_txt_ModifyCustomer.getText();
            String customerPhoneNumber = customerPhoneNumber_txt_ModifyCustomer.getText();
            String customerPostalCode = customerPostalCode_txt_ModifyCustomer.getText();
            String customerCountry = customerCountry_cb_ModifyCustomer.getSelectionModel().getSelectedItem().toString();
            String customerProvince = customerProvince_cb_ModifyCustomer.getSelectionModel().getSelectedItem().toString();
            ...
            ACInventory.deleteCustomer(custToModify);
            int divisionID = 0;
            ...
            try {
                ...
                int result = updateSt.executeUpdate("UPDATE customers SET Customer_Name = '" + customerName + "',Address = '" + customerAddress + "',Postal_Code = '" + customerPostalCode + "',Phone = '" + customerPhoneNumber + "',Division_ID = '" + divisionID + "' WHERE Customer_ID = '" + customerID + "'");
                if (result > 0) {
                    //do nothing
                } else {
                    //do nothing
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        customerCountry_cb_ModifyCustomer.setItems(countriesObsList);
        custToModify = AppointmentsMainController.getTargetCustomer();
        customerID_txt_ModifyCustomer.setText(String.valueOf(custToModify.getCustomerID()));
        customerName_txt_ModifyCustomer.setText(custToModify.getCustomerName());
        customerAddress_txt_ModifyCustomer.setText(custToModify.getCustomerAddress());
        customerPhoneNumber_txt_ModifyCustomer.setText(custToModify.getCustomerPhoneNumber());
        customerPostalCode_txt_ModifyCustomer.setText(custToModify.getCustomerPostalCode());
        customerCountry_cb_ModifyCustomer.setValue(custToModify.getCustomerCountry());
        customerProvince_cb_ModifyCustomer.setValue(custToModify.getCustomerProvince());
    }
}